...
toGeoJSON = (function() {
    ...
    var t = {
        kml: function(doc, o) {
            ...
            function gxCoord(v) { return numarray(v.trim().split(' ')); }
            ...
            function whens(root) {
                var elems = get(root, 'when'), whens = [];
                for (var i = 0; i < elems.length; i++) whens.push(nodeVal(elems[i]));
                return whens;
            }            
            ...
            function getTrackWhens(root) {
                var geomNodes, i, times = [];
                if (get1(root, 'MultiTrack')) return getTrackWhens(get1(root, 'MultiTrack'));
                geomNodes = get(root, "Track");
                if (geomNodes) {
                    for (i = 0; i < geomNodes.length; i++) {
                        times = times.concat(whens(geomNodes[i]));
                    }
                }
                return times;
            }            
        ...
        gpx: function(doc, o) {
            ...
            function getLinestring(node, pointname) {
                var j, pts = get(node, pointname), line = [], times=[];
                for (j = 0; j < pts.length; j++) {
                    line.push(coordPair(pts[j]));
                    times.push(timeProp(pts[j]));
                }
